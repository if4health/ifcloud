...
const {
  getComponentChangeForm,
  processComponentChangeForm,
  getDataFromComponentForm,
} = require("../operations/support/form");
...
class OperationController {
  async operationStarter(req, res) {
    try {
      ...
      const serverError = new HandleServerError("An unexpected error occurred", 500)
  ...
  async myForm(req, res) {
    try {
      ...
      for (let fieldIndex = 0; fieldIndex < changeField.length; fieldIndex++) {
        const componentChange = getComponentChangeForm(
          fhirComponents,
        ...
        const dataFromComponent = getDataFromComponentForm(
          componentChange,
      ...
      const processedData = await processComponentChangeForm(
        arrDataComponents,
        scriptName
      );
      ...
      for (let fieldIndex = 0; fieldIndex < changeField.length; fieldIndex++) {
        ...
        arrComponentsChanges[fieldIndex][changeField[fieldIndex]] = value
          ?.toString()
          .replace(/(\r\n|\n|\r)/gm, "");
      ...
      if (onlyComponent) {
        return res.render("form_example", {
          data: arrFilteredComponents,
          response,
        });
      }
